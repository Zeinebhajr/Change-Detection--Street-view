...
import os
from modules.eval import mAPEvaluator
from modules.eval import BoundingBox, calculate_iou
...
def evaluate_predictions(all_predictions: list, all_ground_truths: list, 
                        image_ids: list, image_type: str, 
                        output_dir: str, iou_thresholds: list = [0.5, 0.75, 0.9]):
    """
    Évalue les prédictions avec les ground truth
    
    Args:
        all_predictions: Liste d'arrays numpy [N, 5] (prédictions)
        all_ground_truths: Liste d'arrays numpy [N, 4] (ground truth)
        image_ids: Liste des IDs d'images
        image_type: Type d'image ("image1" ou "image2")
        output_dir: Dossier de sortie
        iou_thresholds: Seuils IoU à évaluer
    """
    
    # Créer l'évaluateur
    evaluator = mAPEvaluator(iou_thresholds, class_names=["object"])
    
    # Compter les annotations valides
    valid_pairs = 0
    total_gt_boxes = 0
    total_pred_boxes = 0
    
    # Ajouter toutes les prédictions et GT
    for pred_array, gt_array, image_id in zip(all_predictions, all_ground_truths, image_ids):
        if gt_array.size > 0:  # Seulement si on a des GT
            evaluator.add_predictions_from_array(pred_array, image_id)
            evaluator.add_ground_truth_from_array(gt_array, image_id)
            valid_pairs += 1
            total_gt_boxes += len(gt_array)
            total_pred_boxes += len(pred_array)
    
    print(f" Statistiques:")
    print(f"  - Images avec annotations: {valid_pairs}")
    print(f"  - Total GT boxes: {total_gt_boxes}")
    print(f"  - Total prédictions: {total_pred_boxes}")
    
    if valid_pairs == 0:
        print(" Aucune paire prédiction-GT valide trouvée !")
        return None, None
    ...
    # Sauvegarder les courbes P-R
    if results:
        pr_curves_path = os.path.join(output_dir, f"pr_curves_{image_type}.png")
        evaluator.plot_pr_curves(results, pr_curves_path)
        print(f"📈 Courbes P-R sauvegardées: {pr_curves_path}")
    
    return results, evaluator
def calculate_iou_matrix(pred_boxes, gt_boxes):
    """Calculate IoU matrix between predictions and ground truth boxes."""
    if len(pred_boxes) == 0 or len(gt_boxes) == 0:
        return np.zeros((len(pred_boxes), len(gt_boxes)))
    
    # Convert to BoundingBox objects for IoU calculation
    pred_bboxes = [BoundingBox(box[0], box[1], box[2], box[3]) for box in pred_boxes]
    gt_bboxes = [BoundingBox(box[0], box[1], box[2], box[3]) for box in gt_boxes]
    
    iou_matrix = np.zeros((len(pred_bboxes), len(gt_bboxes)))
    
    for i, pred_bbox in enumerate(pred_bboxes):
        for j, gt_bbox in enumerate(gt_bboxes):
            iou_matrix[i, j] = calculate_iou(pred_bbox, gt_bbox)
    
    return iou_matrix

def calculate_per_image_metrics(pred_array, gt_array, iou_threshold):
    """Calculate metrics for a single image."""
    if len(pred_array) == 0 and len(gt_array) == 0:
        return {'mAP': 1.0, 'recall': 1.0, 'precision': 1.0, 'f1': 1.0}
    elif len(pred_array) == 0:
        return {'mAP': 0.0, 'recall': 0.0, 'precision': 0.0, 'f1': 0.0}
    elif len(gt_array) == 0:
        return {'mAP': 0.0, 'recall': 0.0, 'precision': 0.0, 'f1': 0.0}
    
    # Calculate IoU matrix
    pred_boxes = pred_array[:, :4]  # x1, y1, x2, y2
    gt_boxes = gt_array[:, :4] if gt_array.shape[1] >= 4 else gt_array
    
    iou_matrix = calculate_iou_matrix(pred_boxes, gt_boxes)
    
    # Find matches
    matches = np.zeros(len(pred_array), dtype=bool)
    gt_matched = np.zeros(len(gt_array), dtype=bool)
    
    # Sort predictions by confidence (descending)
    if pred_array.shape[1] >= 5:
        sorted_indices = np.argsort(-pred_array[:, 4])  # Sort by confidence
    else:
        sorted_indices = np.arange(len(pred_array))
    ...
    fn = len(gt_array) - np.sum(gt_matched)
    ...
    return {
        'mAP': precision,  # Simplified mAP approximation
        'recall': recall,
        'precision': precision,
        'f1': f1
    }
